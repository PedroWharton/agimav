...
5. **MTBF is dual-mode.** `Mantenimiento.horasAcumuladasSnapshot` (nullable) is written by every `mantenimiento.create` call site (user form, plantilla aplicar, revisión programada child). Legacy rows imported from flota7.db have no snapshot. The estadísticas compute prefers **hour-based MTBF** when every consecutive pair of correctivos has snapshots on both ends; otherwise falls back to **date-based** for the whole row (doesn't mix units within a single row). The column header is plain "MTBF" — the cell renders the unit suffix (`h` / `d`) so mixed-mode rows coexist. Post-cutover, as new correctivos accrue with snapshots, rows transition from date-based to hour-based automatically.
...
8. **Estado literals are centralized** — `MANT_ESTADO_PENDIENTE`, `MANT_ESTADOS_ACTIVOS` (`lib/mantenimiento/estado.ts`), `OT_ESTADO_EN_CURSO` (`app/(app)/ordenes-trabajo/types.ts`), `OC_ESTADOS_ABIERTAS` (`lib/compras/oc-estado.ts`). Maquinaria is still an inline `"activo"` with a warning comment. The casing is **inconsistent across tables** (lowercase for máquinas, title-case with spaces for mant/OT/OC) — full normalization is tracked as a post-cutover backlog item. Do **not** swap to lowercase one-off; it silently breaks every missed callsite.
